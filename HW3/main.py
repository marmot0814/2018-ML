...
def plot(X, Y, weight, bias, keys):
	...
	for frame in range(frames):
		...
		plt.plot([minx,maxx],[minx*weight[frame]+bias, maxx*weight[frame]+bias])
...
def p1(datas, test_datas, keys, index):
	...
	train_data = np.append(const_one_arr, train_data, axis=1)	#append const column
	...
	print("weight: ", lm.coef_[1:])
	print("bias: ",lm.coef_[0])
