...
def Cut(pts, axis, min_x, max_x, min_y, max_y):
	...
	if axis == 0:
		plt.plot([pts[median][0], pts[median][0]],[min_y, max_y], color='red', linewidth=1)
		if median != 0:
		...
		plt.plot([min_x, max_x], [pts[median][1],pts[median][1]], color='blue', linewidth=1)
		if median != 0:
...
Cut(data, cut_axis, min_data[0]-2, max_data[0]+2, min_data[1]-2, max_data[1]+2)
